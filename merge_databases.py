...
def copy_viewerdata():
    ...
    for uid in str_uids:
        sql_username = c2.execute(
            "SELECT User_Name FROM ViewerData WHERE UID=?", (uid[0],)
        )
        str_username = sql_username.fetchone()[0]

        sql_usertype = c2.execute(
            "SELECT User_Type FROM ViewerData WHERE UID=?", (uid[0],)
        )
        ...
        sql_join_message = c2.execute(
            "SELECT Join_Message FROM ViewerData WHERE UID=?", (uid[0],)
        )
        ...
        sql_join_date = c2.execute(
            "SELECT Join_Date FROM ViewerData WHERE UID=?", (uid[0],)
        )
        str_join_date = sql_join_date.fetchone()[0]

        sql_join_date_check = c2.execute(
            "SELECT Join_Date_Check FROM ViewerData WHERE UID=?", (uid[0],)
        )
        ...
        sql_last_seen = c2.execute(
            "SELECT Last_Seen FROM ViewerData WHERE UID=?", (uid[0],)
        )
        ...
        sql_invited_by = c2.execute(
            "SELECT Invited_By FROM ViewerData WHERE UID=?", (uid[0],)
        )
        ...
        sql_join_game = c2.execute(
            "SELECT Join_Game FROM ViewerData WHERE UID=?", (uid[0],)
        )
        ...
        if str(uid[0]) == "12358132":
            ...
            c1.execute(
                "INSERT INTO ViewerData(UID, User_Name, User_Type, Honor, Join_Message, Join_Date, "
...
def copy_hoursdata():
    ...
    for item in str_all_hours_data:
        ...
        c1.execute(
            "INSERT INTO Daily_Stats (Entry_Number, UID, Game, Date, Seconds, Chat) "
            "VALUES(?, ?, ?, ?, ?, ?)",
            (str_entry_number, uid, game, date, seconds, chat),
        )
    ...
    for item in str_all_viewer_chat_data:
        ...
        Game = item[5]
        c1.execute(
            "INSERT INTO viewer_chat (MessageNum, UID, Date, Time, Message, Game) "
            "VALUES(?, ?, ?, ?, ?, ?)",
            (MessageNum, UID, Date, Time, Message, Game),
        )
