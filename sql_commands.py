...
from twitchbot import encryption_key, current_game, twitchchat
...
def create_viewer_tables():
    ...
    c.execute(
        "INSERT INTO ViewerData(UID, User_Name, User_Type, Honor, Points, Join_Date) "
...
def insert_user(User_Name, User_Type, Join_Date, game):
    try:
        ...
        if type(User_Name) is list or User_Name is None:
            ...
            c.execute(
                'INSERT INTO ViewerData(UID, "User_Name", User_Type, Join_Date, Points, Join_Game) Values'
                "(?, LOWER(?), ?, ?, ?, ?)",
                (UID_generator(), str(User_Name), User_Type, Join_Date, 0, game),
            )
        conn.commit()
        conn.close()
...
def get_table_columns():  # startup
    ...
    if "Updating_Name_Point_Deduction" not in columns:
        c.execute(
            "ALTER TABLE ViewerData ADD COLUMN Updating_Name_Point_Deduction INTEGER"
        )
...
def check_table_names():  # startup
    ...
    if "viewer_chat" not in table_list:
        c.execute(
            "CREATE TABLE viewer_chat (MessageNum INTEGER PRIMARY KEY, UID INTEGER, Date STRING, "
            "Time STRING, Message STRING, Game STRING)"
        )
    if "Daily_Stats" not in table_list:
        c.execute(
            "CREATE TABLE Daily_Stats (Entry_Number INTEGER PRIMARY KEY, UID INTEGER, Game STRING, Date STRING, "
            "Seconds INTEGER, Chat INTEGER)"
        )
    if "Error_Log" not in table_list:
        c.execute(
            "CREATE TABLE Error_Log (Entry_Number INTEGER PRIMARY KEY, Date STRING, "
...
def error_log_writing(general):
    ...
    if general.errors is not None:
        for i in general.errors:
            ...
            c.execute(
                "INSERT INTO Error_Log (Entry_Number, Date, Time, Error, Patch_Number) VALUES(?, ?, ?, ?, ?)",
...
def UID_generator():  # this needs to read the last uid from the database and use the algorithm on
    ...
    secondtolast_char = str(
        algorithm[-2:-1:]
    ).strip()  # take second to last character of algorithm
    ...
    for viewer in copy_of_viewerobjects:
        ...
        if uid is None or uid == "None":
            ...
            for game in copy_of_viewerobjects[viewer].seconds:
                ...
                sql_gameuid = c1.execute(
                    "SELECT UID FROM Daily_Stats WHERE UID=? AND Game=? AND Date=?",
                    (uid, game, todaydate),
                )
                string_gameuid = sql_gameuid.fetchone()
                if string_gameuid is None:
                    sql_entry_number = c1.execute(
                        "SELECT MAX(Entry_Number) FROM Daily_Stats"
                    )
                    str_entry_number = sql_entry_number.fetchone()
                    if str_entry_number[0] is None:
                        ...
                        str_entry_number = str_entry_number[0] + 1
                    c1.execute(
                        "INSERT INTO Daily_Stats(Entry_Number, UID, Game, Date, Seconds) Values(?, ?, ?, ?, ?)",
                        (str_entry_number, uid, game, todaydate, seconds),
                    )
                else:

                    sql_oldtime = c1.execute(
                        "SELECT Seconds FROM Daily_Stats WHERE UID=? AND Game=? AND Date=?",
                        (uid, game, todaydate),
                    )
                    string_oldtime = sql_oldtime.fetchone()

                    c1.execute(
                        "UPDATE Daily_Stats SET Seconds=?+? WHERE UID=? AND Game=? AND Date=?",
                        (string_oldtime[0], seconds, uid, game, todaydate),
                    )
...
def update_invited_by(general):
    ...
    for viewer in copy_of_viewerobjects:
        if copy_of_viewerobjects[viewer].invited_by is not None:
            c.execute(
                "UPDATE ViewerData SET Invited_By=? WHERE User_Name=?",
                (general.viewer_objects[viewer].invited_by, viewer),
            )
    conn.commit()
    conn.close()
...
def update_user_chat_lines(
    date, general
):  # this should grab an item from viewerclass and add that to game for day
    conn1 = sqlite3.connect(new_sql_file())
    c1 = conn1.cursor()
    ...
    for viewer in copy_of_viewerobjects:
        ...
        for game in copy_of_viewerobjects[viewer].chat_line_dict:
            ...
            sql_old_chatlines = c1.execute(
                "SELECT Chat FROM Daily_Stats WHERE UID=? AND Game=? AND Date=?",
                (uid, game, date),
            )
            ...
            chat_line_total = old_chatlines + copy_of_viewerobjects[
                viewer
            ].chat_line_dict.get(game)
            c1.execute(
                "UPDATE Daily_Stats SET Chat=? WHERE UID=? AND Game=? AND Date=?",
                (chat_line_total, uid, game, date),
            )
...
def update_user_points(general):
    ...
    for viewer in copy_of_viewerobjects:
            ...
            for game in copy_of_viewerobjects[viewer].seconds:
                ...
                if game == "Offline":
                    ...
                    general.viewer_objects[viewer].honor += (
                        general.viewer_objects[viewer].seconds.get(game) * 0.005
                    )
                else:
                    points = 0.016 * general.viewer_objects[viewer].seconds.get(game)
                    general.viewer_objects[viewer].honor += (
                        general.viewer_objects[viewer].seconds.get(game) * 0.05
                    )
                sql_oldpoints = c.execute(
                    "SELECT Points FROM ViewerData WHERE User_Name=?", (viewer,)
                )
                string_oldpoints = sql_oldpoints.fetchone()[0]
                total_points = (
                    float(string_oldpoints)
                    + float(points)
                    + general.viewer_objects[viewer].points
                )
                c.execute(
                    "UPDATE ViewerData SET Points=? WHERE User_Name=?",
                    (total_points, viewer),
                )
        ...
        if general.honor_bool is True:
            sql_old_honor = c.execute(
                "SELECT Honor FROM ViewerData WHERE User_Name=?", (viewer,)
            )
            ...
            c.execute(
                "UPDATE ViewerData SET Honor=? WHERE User_Name=?",
                (combined_honor, viewer),
            )
...
def update_trivia_points(general):
    ...
    for username in general.viewer_objects:
        ...
        if trivia_points > 0:
            sql_trivia_points = c.execute(
                "SELECT Trivia_Answers FROM ViewerData WHERE User_Name=?", (username,)
            )
            ...
            c.execute(
                "UPDATE ViewerData SET Trivia_Answers=? WHERE User_Name=?",
                (str_trivia_points, username),
            )
...
def update_bots():  # time based
    ...
    sql_bots = c1.execute(
        "SELECT DISTINCT UID FROM Daily_stats WHERE Game NOT IN ('Offline') "
    ...
    other_bots = [
        "nightbot",
        "moobot",
        encryption_key.decrypted_nick,
        "zerg3rrbot",
        "giphertius",
    ]
    for i in other_bots:
        c1.execute(
            "UPDATE ViewerData Set User_Type = 'Botter' WHERE User_Name = ?", (i,)
        )
...
def update_last_seen(general):
    ...
    for username in general.viewer_objects:
        ...
        if last_seen is not None:
            ...
            c.execute(
                "UPDATE ViewerData SET Last_Seen = ? WHERE User_Name = ?",
    ...
    for viewer in copy_of_viewerobjects:
        if copy_of_viewerobjects[viewer].chat is not None:
            ...
            for chat_item in copy_of_viewerobjects[viewer].chat:
                ...
                c.execute(
                    "INSERT INTO viewer_chat(MessageNum, Date, Time, UID, Message, Game) Values(?,?,?,?,?,?)",
                    (
                        int_currnum,
                        chat_item[0],
                        chat_item[1],
...
def welcome_viewers(
    s, general, getviewers, currtime
):  # welcomes all new viewers with a joinmessage
    conn = sqlite3.connect(new_sql_file())
    c = conn.cursor()
    for viewer in getviewers:
        if viewer not in general.viewer_objects:
            ...
            sql_check_for_joinmessage = c.execute(
                "SELECT Join_Message FROM ViewerData WHERE User_Name=?", (viewer,)
            )
            str_check_for_joinmessage = sql_check_for_joinmessage.fetchone()
            if (
                str_check_for_joinmessage[0] is None
                or str_check_for_joinmessage[0] == "None"
            ):
                ...
                > 2400
                or general.viewer_objects[viewer].last_seen == 0
            ):

                twitchchat.chat(s, str(str_check_for_joinmessage[0]))
                general.viewer_objects[viewer].last_seen = currtime
                general.viewer_objects[
                    viewer
                ].time_before_last_seen = general.viewer_objects[viewer].last_seen
            else:
                general.viewer_objects[viewer].join_message_check = False
    conn.close()
...
def write_welcome_viewers(general):
    ...
    for viewer in general.viewer_objects:
        if (
            general.viewer_objects[viewer].join_message_check is False
            or general.viewer_objects[viewer].join_message_check is None
        ):
            pass
        elif general.viewer_objects[viewer].join_message_check == "remove_joinmessage":
            c.execute(
                "UPDATE ViewerData Set Join_Message = ? WHERE User_Name = ?",
                (None, viewer),
            )
            ...
            c.execute(
                "UPDATE ViewerData Set Join_Message = ? WHERE User_Name = ?",
                (general.viewer_objects[viewer].join_message_check, viewer),
            )
...
def check_users_joindate(get_viewers):  # time based
    def get_user_id(username, client_id):
        try:
            ...
            personal_user_info = requests.get(
                "https://api.twitch.tv/helix/users?login={}".format(username),
                headers=headers,
            ).json()
    ...
    def initial_follow_date_for(username, client_id):
        ...
        try:
            follow_data = requests.get(
                "https://api.twitch.tv/helix/users/follows?from_id={}&to_id={}".format(
                    user_id, get_user_id(encryption_key.decrypted_chan, client_id)
                ),
                headers=headers,
            ).json()["data"][0]
        ...
        if get_viewers is not None:
            ...
            if viewer_list is not None:
                for i in viewer_list:
                    sql_jdc = c.execute(
                        "SELECT Join_Date_Check FROM ViewerData WHERE User_Name = ?",
                        (i.strip(),),
                    )
                    string_jdc = sql_jdc.fetchone()
                    if string_jdc[0] != "x":
                        web_date = initial_follow_date_for(
                            i.strip(), client_id=str("j82z1o73ha4tauoxb8y462udh8t4i2")
                        )
                        if web_date is False:
                            c.execute(
                                'UPDATE ViewerData SET Join_Date_Check = "x" '
                                "WHERE User_Name = ?",
                                (i,),
                            )
                            conn.commit()
                        if web_date:
                            ...
                            sql_users_joindate = c.execute(
                                "SELECT Join_Date FROM ViewerData WHERE User_Name = ?",
                                (i,),
                            )
                            ...
                            if users_joindate is not None:
                                ...
                                full_strnow = str(
                                    str_yearnow + "-" + str_monthnow + "-" + str_daynow
                                )
                                full_webdate = (
                                    str(web_date[0])
                                    + "-"
                                ...
                                if users_joindate != year_month_day[0]:
                                    if year_month_day[0] != "":
                                        c.execute(
                                            "UPDATE ViewerData SET Join_Date = ? WHERE User_Name = ?",
                                            (year_month_day[0], i),
                                        )
                                        c.execute(
                                            'UPDATE ViewerData SET Join_Date_Check = "x" '
                                    ...
                                    c.execute(
                                        'UPDATE ViewerData SET Join_Date_Check = "x" WHERE User_Name=?',
                                        (i,),
                                    )
...
def check_mods(general):  # bots should be able to be mods
    ...
    sql_mod_list = c.execute(
        "SELECT User_Name FROM ViewerData WHERE User_Type = 'Moderator'"
    )
        ...
        if general.get_viewers_func[1]:
            for i in general.get_viewers_func[1]:
                if i in mod_list:
                    ...
                    mod_list.append(i)
    for i in mod_list:
        sql_find_user = c.execute(
            "SELECT User_Name FROM ViewerData WHERE User_Name = ?", (i,)
        )
        ...
        if not user:
            ...
            if i == encryption_key.decrypted_chan.lower():
                c.execute(
                    "UPDATE ViewerData SET User_Type = 'Streamer' WHERE User_Name = ?",
                    (i,),
                )

            c.execute(
                "UPDATE ViewerData SET User_Type = 'Moderator' WHERE User_Name = ?",
                (i,),
            )
        ...
        if i not in general.get_viewers_func[1]:
            if i == "zerg3rr" or i == encryption_key.decrypted_chan:
                ...
                if i in general.get_viewers_func[0]:
                    c.execute(
                        "UPDATE ViewerData SET User_Type='Viewer' WHERE User_Name=?",
                        (i,),
                    )
...
def combine_db_data(general, username):
    ...
    if str_joindate is not None:
        c.execute(
            "UPDATE ViewerData SET Join_Date=? WHERE UID=?",
            (str_joindate[0], str_new_uid),
        )

    sql_points = c.execute("SELECT Points FROM ViewerData WHERE UID=?", (old_uid,))
    str_points = sql_points.fetchone()
    c.execute(
        "UPDATE ViewerData SET Points=Points+? WHERE UID=?",
        (str_points[0], str_new_uid),
    )

    sql_invited_by = c.execute(
        "SELECT Invited_By FROM ViewerData WHERE UID=?", (old_uid,)
    )
    str_invited_by = sql_invited_by.fetchone()
    if str_invited_by is not None:
        c.execute(
            "UPDATE ViewerData SET Invited_By=? WHERE UID=?",
            (str_invited_by[0], str_new_uid),
        )

    sql_join_game = c.execute(
        "SELECT Join_Game FROM ViewerData WHERE UID=?", (old_uid,)
    )
    str_join_game = sql_join_game.fetchone()
    if str_join_game is not None:
        c.execute(
            "UPDATE ViewerData SET Join_Game=? WHERE UID=?",
            (str_join_game[0], str_new_uid),
        )

    sql_point_check = c.execute(
        "SELECT Updating_Name_Point_Deduction FROM ViewerData WHERE UID=?", (old_uid,)
    )
    str_join_game = sql_point_check.fetchone()
    if str_join_game is None:
        c.execute(
            "UPDATE ViewerData SET Updating_Name_Point_Deduction=200 WHERE User_Name=?",
            (username,),
        )
    else:
        c.execute(
            "UPDATE ViewerData SET Updating_Name_Point_Deduction=Updating_Name_Point_Deduction*2 "
            "WHERE User_Name=?",
            (username,),
        )
    ...
    c.execute(
        "UPDATE ViewerData SET Points=Points-Updating_Name_Point_Deduction WHERE UID=?",
        (str_new_uid,),
    )
